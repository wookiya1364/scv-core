...
| 계약 | 버전 | 의미 |
|---|---:|---|
| SCV Core | `0.35.0` | 공통 동작과 릴리스 페이로드 |
| Core API | `1` | 래퍼와 코어의 통합 계약 |
| Template | `2.3.0` | hydrate되는 프로젝트 템플릿 스키마 |
...
이제 명령이 유일한 입구가 아닙니다(0.35.0+): 매 턴 훅이 일반 대화를 help
액션으로 라우팅합니다 — 프로젝트 설정 파일의 `SCV_ALWAYS_ON=off` 만이 이를
끕니다. 같은 훅이 쉬운말 답 모양 알림(0.31.0+)도 실어 나릅니다. 프로젝트
설정은 `scv/scv_settings.json`(+ git 무시되는 비밀 파일)에 있으며, 모든 키가
설명과 함께 자동 생성됩니다(0.34.0+). 프로젝트의 `.env` 는 읽지 않습니다.
...
호스트별 재생성, 회귀 테스트를 거쳐 `develop` 대상 PR을 엽니다. 자세한 내용은
