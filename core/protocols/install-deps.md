...
Technical identifiers (tool names like `gh`, `glab`, `ffmpeg`, command names, file paths, env var names) stay as-is. If `scv/scv_settings.json` `SCV_LANG` is unset, suggest `action:help` once to lock the preference (don't block — fall back to auto-detect / English for now).
