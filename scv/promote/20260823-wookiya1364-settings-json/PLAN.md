...
scope:
  ...
  - "core/template/scv/scv_settings.json" # 신설 (기본값 예시)
...
### 1단계 — 읽는 입구를 모은다 (동작 변화 없음)

- `core/scripts/lib/settings.sh` 신설. 위 세 함수.
- 이 시점의 `settings_get` 은 **여전히 `.env` 를 읽는다.**
- 직접 읽던 24군데를 `settings_get` 호출로 교체.
- 프로토콜 문서의 직접 읽기 안내도 교체.

**완료 판정**: 기존 테스트 스위트 전부 통과. 동작이 하나도 안 바뀌었다는 뜻이다.

### 2단계 — 저장소를 갈아끼운다

- `settings_get` **안쪽만** 바꾼다: `.env` → 두 JSON.
- `core/template/scv/scv_settings.json` 신설 (기본값이 주석처럼 보이는 예시).
- 무시 규칙에 비밀 설정 파일 추가.
- `core/scripts/settings-set.sh` 신설 — 기존 쓰기 스크립트를 대체. 무관한 키는
  건드리지 않고 값 하나만 바꾼다.
- **이전 절차**: `.env` 가 있고 새 설정 파일이 없으면, 36개 키를 읽어 두 JSON 으로
  나눠 쓴다. 원본은 지우지 않고 그대로 둔다.

**완료 판정**: 새 파일만으로 모든 값이 읽힌다. 비밀 설정 파일이 무시 목록에 걸린다.

### 3단계 — 잔재를 지운다

- `core/template/.env.example.scv` 제거.
- `core/scripts/env-set.sh`, `core/scripts/lib/env.sh` 제거.
- 전용 테스트 두 개 제거 또는 새 대상으로 교체.
- 문서 16개에서 `.env` 언급 제거.
- 래퍼 두 곳의 스크립트 사본·문서 반영.

**완료 판정**: 저장소 어디에도 `.env` 를 설정으로 읽는 코드가 없다.
...
- **템플릿 버전**을 어디까지 올릴지. 예시 파일이 사라지고 새 파일이 생기므로
  뒤바뀜이 있는 변경이다. 권고는 큰 자리 올림.
- **채널 ID 를 비밀 쪽에 두는 게 맞는지.** 팀에 따라 공유하고 싶을 수 있다.
  지금은 안전한 쪽으로 두고, 필요하면 나중에 옮긴다.
