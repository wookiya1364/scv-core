...
invariants:
  ...
  - "sync 의 삭제는 --dry-run 에서 예고되고 CHANGES 에 DELETED 로 보고된다 (백업 없음 — 사용자 결정)"
...
2. **sync 삭제 전파** (사용자 결정 — 백업 없음): 기존 프로젝트에서 sync
   실행 시 위 7종이 존재하면 **즉시 삭제**하고 `CHANGES` 에 `DELETED <file>`
   로 보고한다. `--dry-run` 은 삭제 예정 목록을 표시만 한다. 프로토콜
   (sync.md)은 host agent 에게 지시: sync 실행 **전에** 각 파일에 실질
   내용(사용자 작성)이 보이면 "보존할 결정은 DECISIONS.md 로 이관"을 먼저
   제안한 뒤 진행한다 (team-journal 이 같은 웨이브에 포함되는 이유).
...
새로 도입하고, `--dry-run` 은 삭제 예정 목록만 출력한다 (백업 없음 —
사용자 결정). REPORTING/TESTING
...
- **데이터 손실**: 사용자가 실제 내용을 채운 문서도 백업 없이 삭제된다
  (사용자 결정). 완화는 두 겹: sync.md 가 실행 전 이관 제안을 지시,
  git 이력이 사실상의 복구 수단 (릴리스 노트에 명시).
