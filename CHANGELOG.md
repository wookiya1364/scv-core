...
### 고정 밀리초 예산 제거 — 부하가 초록·붉음을 정하던 문제

검사 셋(`test-graph` · `test-answer-lint-source` · `test-graft-adapter`)에 벽시계 예산이 밀리초로 박혀
있었다. 의도는 알고리즘이 느려지는 것을 잡는 것이었지만, 기계가 바쁘면 코드와 무관하게 무너졌다. 이 검사들은
아카이브 계획 25건이 공유하는 관문 안에서 돌기 때문에, 부하 한 번에 25건이 한꺼번에 붉었다. 실제로 같은 관문이
한가할 때 348초에 통과하고 바쁠 때 431초에 무너졌으며, 부하 중 그래프 빌드는 2,326ms 로 2,000ms 예산을 넘었다.

`core/tests/lib/timing.sh` 가 source 시점에 작은 고정 작업을 한 번 재서 이 셸이 기준보다 몇 배 굼뜬지 구하고,
예산에 그 배수를 곱한다. 한가한 기계에서는 배수가 1이라 예산이 그대로 남아 진짜 느려짐은 여전히 잡히고, 바쁜
기계에서는 함께 늘어난다. 배수에는 상한(기본 8)이 있어 아주 바쁜 기계에서 어떤 느림도 통과하지는 않는다.
CI 재현용으로 `SCV_TEST_TIME_SCALE` 로 배수를 고정할 수 있다. 측정은 한 번뿐이다 — 물을 때마다 재면 같은
실행 안에서 기준이 흔들려, 고치려던 병을 다시 만든다.
